...
export function initControls() {
  ...
  if (auroraButton) auroraButton.addEventListener('click', function () {
    state.auroraEnabled = !state.auroraEnabled;
    auroraButton.textContent = state.auroraEnabled ? 'Aurora On' : 'Aurora Off';
    auroraButton.classList.toggle('active', state.auroraEnabled);
  });
  ...
  for (let i = 0; i < qualityButtons.length; i++) {
    ...
    qualityButtons[i].addEventListener('click', function () {
      location.hash = 'quality=' + this.getAttribute('data-q');
      location.reload();
    });
  }
}

const moveState = { forward: 0, strafe: 0, manual: false };
...
export function updatePlayer(dt) {
  ...
  if (state.running && !moveState.manual) {
    // Run button = an actual auto-run mode. The character keeps moving in
    // the direction it was already facing until the player steers it.
    _characterForward.set(0, 0, -1).applyAxisAngle(new THREE.Vector3(0, 1, 0), character.rotation.y);
    moveDelta.copy(_characterForward);
  } else {
    moveDelta.addScaledVector(dirs.forward, moveState.forward);
    moveDelta.addScaledVector(dirs.right, moveState.strafe);
  }
  ...
  for (let i = 0; i < state.trees.length; i++) {
    ...
    if (Math.hypot(nx - t.x, nz - t.z) < t.radius + 0.5) {
      nx = character.position.x;
      nz = character.position.z;
      blocked = true;
      break;
    }
  }

  const dx = nx - character.position.x;
  const dz = nz - character.position.z;
  ...
  if (moving && (dx * dx + dz * dz) > 0.000001) {
    const desiredFacing = Math.atan2(-dx, -dz);
    character.rotation.y = lerpAngle(character.rotation.y, desiredFacing, state.running ? 0.18 : 0.22);
  } else if (state.running && !moveState.manual && blocked) {
    // Keep autonomous running from getting glued to a house/tree.
    character.rotation.y += 0.85 * dt;
  }

  updateCharacterAnim(dt, moving, state.running);
}

const followDistance = 5.5;
const pivotHeight = 1.25;
const orientHelper = new THREE.Object3D();
orientHelper.rotation.order = 'YXZ';
