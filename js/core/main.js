...
function init() {
  ...
  let renderer;
  try { renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: !isMobile, powerPreference: 'high-performance' }); }
  catch (e) { document.getElementById('loading').textContent = 'Your browser does not support WebGL, which this scene needs.'; return; }
  ...
  function animate() {
    ...
    elapsed += dt;
    if (!state.timePaused) state.dayTime += dt / cycleSeconds;
